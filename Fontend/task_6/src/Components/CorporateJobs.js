...
const FreelanceJobs=()=>{
    ...
    useState(()=>{
        axios.get("http://127.0.0.1:8000/api/Freelance/PostedJobs/Corporate/").then(resp=>{
            ...
            <p  style={{textAlign:"center"}}>Corporate Jobs   </p> 
                {
                   posts.map(p=>(
                    ...
                    <li key={p.Post_id}> Title: <Link to={"/corporate/posts/details/"+p.Post_Id}>{p.Post_Title} </Link><br/>Salary: {p.Salary}
